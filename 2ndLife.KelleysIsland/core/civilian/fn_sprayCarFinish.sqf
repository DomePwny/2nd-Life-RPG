...
while{true} do {
	hint parsetext format["<img size='1' image='icons\info.paa'/> <t color='#FFCC00'><t size='0.75'>JOB:</t><br/> Stay Still! Painting: %1 seconds.",_remaining];
	uisleep 1;
...
if(_complete) then {
	...
	hint parsetext format["<img size='1' image='icons\info.paa'/> <t color='#FFCC00'><t size='0.75'>JOB:</t><br/>Spray Guy: Completed!",_remaining];
} else {
	hint parsetext format["<img size='1' image='icons\info.paa'/> <t color='#FFCC00'><t size='0.75'>JOB:</t><br/>Spray Guy: Failed!",_remaining];	
};