...
switch (_code) do
{
	...
	case 2:
	{
		...
		if(_shift) then
		{
			if( playerSide == west && vehicle player != player && ((driver vehicle player) == player) ) then
			{
				...
				{
					[[_veh, "StopVehicle1"],"life_fnc_say3d",true,false] call BIS_fnc_MP;
					[] spawn life_fnc_AntiSpam3;
				};
			};
	...
	case 3:
	{
		...
		if(_shift) then
		{
			if( playerSide == west && vehicle player != player && ((driver vehicle player) == player) ) then
			{
				...
				if( _veh isKindOf "Car" || _veh isKindOf "Motorcycle" || _veh isKindOf "Motorbike" ) then
				{
					[[_veh, "StopVehicle2"],"life_fnc_say3d",true,false] call BIS_fnc_MP;
					[] spawn life_fnc_AntiSpam3;
				};
			};
	...
	case 4:
	{
		if(_shift) then
		{
			if( playerSide == west && vehicle player != player && ((driver vehicle player) == player) ) then
			{
				...
				if( _veh isKindOf "Car" || _veh isKindOf "Motorcycle" || _veh isKindOf "Motorbike" ) then
				{
					[[_veh, "StopVehicle3"],"life_fnc_say3d",true,false] call BIS_fnc_MP;
					[] spawn life_fnc_AntiSpam3;
				};
			};
			_handled = true;
	...
	case 19:
	{
		...
		{
			if( "Second_Life_Binder_i" in magazines player ) then {
				[] spawn life_fnc_AntiSpam2;
	...
	{	
		...
		if(count _radios > 0 && ([(call TFAR_fnc_activeSwRadio),"2nd_lifetabletd"] call TFAR_fnc_isSameRadio) ) then {
			if(life_battery < 1) then {
				if ( !_alt && !_ctrlKey ) then {
					["Keine Batterie, CTRL+Z für das Essen abzurufen | ALT+Z für die Schlüssen abzurufen | CTRL+8 zum Speichern", false] spawn doquickmsg; 
					_stop = true;
					_handled = true;
					[] spawn life_fnc_AntiSpam3;
				};
		...
		if(count _radios == 0) then {
			if( !_alt && !_ctrlKey ) then {
				["Kein Tablet, CTRL+Z für das Essen abzurufen | ALT+Z für die Schlüssen abzurufen | CTRL+8 zum Speichern", false] spawn doquickmsg; 
				_stop = true;
				_handled = true;
				[] spawn life_fnc_AntiSpam3;
			};
		...
		if(!_alt && _ctrlKey && !dialog && !_stop && !(player getVariable ["restrained", false])) then
		{
			...
			if(_veh isKindOf "House_F" && (playerSide == civilian OR playerSide == east)) then {
				if(_veh in life_vehicles && player distance _veh < 8) then {
					...
					if(_locked == 0) then {
						...
						["Haus abgeschlossen!", false] spawn doquickmsg; 
						[] spawn life_fnc_AntiSpam2;
					} else {
						_veh setVariable[format["bis_disabled_Door_%1",_door],0,true];
						_veh animate [format["door_%1_rot",_door],1];
						[ "Haus aufgeschlossen!", false] spawn doquickmsg; 
						[] spawn life_fnc_AntiSpam2;
					};
				};
			} else {
				...
				if(_veh in life_vehicles && player distance _veh < 8 && (velocity _veh select 2) < 20) then {
					...
					if(_veh getVariable ["parkingTicket2", false]) then {
						...
						["STRAFE: 1500$ für Falschparken!", false] spawn domsg;
					...
					if(_veh getVariable ["parkingTicket", false]) then {
						...
						["STRAFE: 500$ für Falschparken!", false] spawn domsg;
