...
hud_fnc_loop =
{
	...
	if(!im_dead) then
	{
		...
		if(player getVariable["severeInjuries",FALSE]) then
		{
			...
			_injurytext = "Sehr starke schmerzen";
		...
		if(koildebi > 1) then
		{
			_woundedon = "Körper Schnittwunden";
			im_hurt = true;
			if(koildebi == 2) then
			{
				_woundedon = "Körper Schnittwunden";
