...
_Btn1 ctrlSetText format["Tabletten verabreichen", life_hunger];
_Btn1 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_coolPlayer; closeDialog 0;";
if((_disease) > 1 && (life_inv_tabletten > 0)) then { _Btn1 ctrlEnable true;} else {_Btn1 ctrlEnable false;};

_Btn2 ctrlSetText format["Genitalbereich eincremen", life_hunger];
_Btn2 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_heatPlayer; closeDialog 0;";
if((_disease) > 1 && (life_inv_genitalcreme > 0)) then { _Btn2 ctrlEnable true;} else {_Btn2 ctrlEnable false;};

_Btn3 ctrlSetText format["Antibiotika verabreichen", life_hunger];
_Btn3 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_HydratePlayer; closeDialog 0;";
if((_disease) > 1 && (life_inv_antibiotika > 0)) then { _Btn3 ctrlEnable true;} else {_Btn3 ctrlEnable false;};

_Btn4 ctrlSetText format["Antibiotika verabreichen", life_hunger];
_Btn4 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_AntiViralPlayer; closeDialog 0;";
if((_disease) > 1 && (life_inv_antibiotika > 0)) then { _Btn4 ctrlEnable true;} else {_Btn4 ctrlEnable false;};

_Btn5 ctrlSetText format["Antibiotika und Tabletten verabreichen", life_hunger];
_Btn5 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_painkillerPlayer; closeDialog 0;";
if((_disease) > 1 && (life_inv_tabletten > 0) && (life_inv_antibiotika > 0)) then { _Btn5 ctrlEnable true;} else {_Btn5 ctrlEnable false;};
