...
switch (true) do
{
	...
	case (_item == "panicbutton"): 
	{
		...
		{
			[_item] spawn life_fnc_panicButton;
			closeDialog 0;
		};
	};
	...
	case (_item == "weddingring"):
	{
		if( life_married != "-2" ) then 
		{
			if(([false,_item,1] call life_fnc_handleInv)) then
			{
				["Überragend! Jemand möchte dich heiraten!", false] spawn domsg;
				if(life_married == "-1") then {
					life_married = "someone";
				};
				if(life_married != "") then {
				[0,format["%1 heiratet %2! Wir wünschen euch das beste!",profileName, life_married]] remoteExecCall ["life_fnc_broadcast", -2];
				life_married = "-2";
				};
			};
		}
		...
		{
			["Du kannst nicht nochmal Heiraten!", false] spawn domsg;
		};
	};
