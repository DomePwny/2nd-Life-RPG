...
if(playerSide == west) then
{
	...
	_message = format["<color='#FFFFFF'/><t size='2'>%1</t><br/><br/>%2<br/>", name player,_id];
...
if(playerSide == east) then
{
	...
	_id = "<img image='idcards\AusweisCiv.paa' size='8'/>";
	_pkw = if(license_maf_driver) then {"<img image='idcards\AusweisCar.paa' size='6'/>";} else {"<br/>"}; 
	_lkw = if(license_maf_truck) then {"<img image='idcards\AusweisTruck.paa' size='6'/>";} else {"<br/>"};
