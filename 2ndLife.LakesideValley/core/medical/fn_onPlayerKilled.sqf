...
if(_length > 5 && _meds < 1) then { _length = 5; };
if(_length > 15 && _meds > 3) then { _length = 15; };
if(_length < 8 && _meds > 1) then { _length = 8; };
...
if(_fuck != _you) then {
	if(_fuck find "Error: " != -1) then {
		[format["%1 ist sehr stark verletzt!", _you], false] spawn domsg; 
		[player,"pain2"] spawn life_fnc_nearestSound;
		shooting_death = false;
	} else {
		[format["%1 erschoss %2 mit einer Distanz von %3 mit der Waffe: %4.", _fuck, _you, _killdistance, _killweapon], false] spawn domsg; 
	...
	shooting_death = false;
	[format["%1 blutet aus!", _fuck], false] spawn domsg; 
	[player,"pain2"] spawn life_fnc_nearestSound;
};
...
_unit spawn
{
	...
	if(shooting_death && round(maxTime - time) <= 0) exitwith { closeDialog 0; life_respawned = true; [] call life_fnc_spawnMenu; [getPlayerUID player] remoteExec ["life_fnc_wantedRemove",2];	};			
};
