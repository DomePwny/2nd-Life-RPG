...
switch (playerSide) do
{
	case independent:
	{
		...
		//Ausweis
		life_actions = life_actions + [player addAction["<t color='#FF00FF'>ID Card übergeben</t>",life_fnc_copShowLicense,"",1,false,true,"",' playerSide == independent && !isNull cursorTarget && player distance cursorTarget < 2.5 && cursorTarget isKindOf "Man" && alive cursortarget ']];
		...
		//Ungag person
		life_actions pushBack (player addAction["<t color = '#d02b2b'>Klebeband entfernen</t>",life_fnc_ungag,"",0,false,false,"",'
		...
		life_actions pushBack (player addAction["<t color='#ADFF2F'>Bänker</t>",life_fnc_atmMenu,"",7,false,false,"",' (cursorTarget getVariable ["ATM_MAN", false]) && cgbankvault animationPhase "d_l_Anim" == 0 && (player distance (getMarkerPos "bank_signup")) < 21 ']);
		...
		//Unblindfold person
		life_actions pushBack (player addAction["Augenbinde entfernen",life_fnc_untieb,"",0,false,false,"",'!isNull cursorTarget && player distance cursorTarget < 2.5 && isPlayer cursorTarget && (cursorTarget getVariable ["blindfolded", false]) ']);
		...
		//Pickup Item
		life_actions pushBack (player addAction["Gegenstände aufheben",{createDialog "life_pickup_items"},"",0,false,false,"",
		...
		//Person behandeln
		life_actions pushBack (player addAction["Selbstbehandeln",life_fnc_healself,"",0,false,false,"",' (player getVariable "playerHealth") > 0 && !isdragging && (life_inv_bandage > 0)']);

		//Heal Self
		life_actions pushBack (player addAction["Person behandeln",life_fnc_healperson,"",0,false,false,"",' !isNull cursorTarget && player distance cursorTarget < 2.5 && isPlayer cursorTarget && (cursorTarget getVariable "playerHealth") > 0 && !isdragging && (life_inv_bandage > 0)']);

		//Untie person
		life_actions pushBack (player addAction["Person entfesseln",life_fnc_untie,"",0,false,false,"",'
		!isNull cursorTarget && player distance cursorTarget < 2.5 && isPlayer cursorTarget && (cursorTarget getVariable ["tied", false]) ']);

		// Heli Schnell abseilen
		life_actions pushBack (player addAction["Schnell abseilen",life_fnc_fastRope,"",99,false,false,"", ' (vehicle player) != player && !isNull (vehicle player) && (vehicle player) isKindOf "Air" && driver (vehicle player) != player && (getPos player) select 2 <= 100 && (getPos player) select 2 >= 15 && speed vehicle player < 30 && !(player getVariable["transporting",false]) ']);
		//Selbstbehandlung Krankheit
		life_actions pushBack (player addAction[localize "STR_pAct_sContPlayer","","",0,false,false,"",' (player getVariable ["diseaseon", 1]) > 1 ']);

		life_actions pushBack (player addAction[localize "STR_pAct_sHeatPlayer",life_fnc_sheatPlayer,"",0,false,false,"",' (player getVariable ["diseaseon", 1]) == 5 && (life_inv_genitalcreme > 0)']);

		life_actions pushBack (player addAction[localize "STR_pAct_sCoolPlayer",life_fnc_scoolPlayer,"",0,false,false,"",' (player getVariable ["diseaseon", 1]) == 3 && (life_inv_tabletten > 0)']);

		life_actions pushBack (player addAction[localize "STR_pAct_sHydratePlayer",life_fnc_shydratePlayer,"",0,false,false,"",' (player getVariable ["diseaseon", 1]) == 2 && (life_inv_antibiotika > 0)']);

		life_actions pushBack (player addAction[localize "STR_pAct_sAntiViralPlayer",life_fnc_santiviralPlayer,"",0,false,false,"",' (player getVariable ["diseaseon", 1]) == 4 && (life_inv_antibiotika > 0)']);
		
		life_actions pushBack (player addAction[localize "STR_pAct_Influenza",life_fnc_influenza,"",0,false,false,"",' (player getVariable ["diseaseon", 1]) == 6 && (life_inv_tabletten > 0)']);

		life_actions pushBack (player addAction[localize "STR_pAct_sPainKillerPlayer",life_fnc_spainkillerPlayer,"",0,false,false,"",' (player getVariable ["diseaseon", 1]) > 6 && (life_inv_antibiotika > 0) && (life_inv_tabletten > 0) ']);

		//healself system
		life_actions pushBack (player addAction[localize "STR_pAct_BandageSelf",life_fnc_bandageSelf,"",0,false,false,"",' (player getVariable ["bleedingon", 1]) > 1 && (life_inv_bandage > 0)']);

		life_actions pushBack (player addAction[localize "STR_pAct_StitchSelf",life_fnc_stitchSelf,"",0,false,false,"",' (player getVariable ["woundedon", 1]) > 1 && (life_inv_stitches > 0)']);

		life_actions pushBack (player addAction[localize "STR_pAct_SplintSelf",life_fnc_splintSelf,"",0,false,false,"",' (player getVariable ["fractureon", 1]) > 1 && (life_inv_splint > 0)']);

		life_actions pushBack (player addAction[localize "STR_pAct_EpiPenSelf",life_fnc_epipenSelf,"",0,false,false,"",' (player getVariable ["unconciouson", 1]) > 1 && (life_inv_epipen > 0)']);
	...
	case east:
	{
		...
		life_actions pushBack (player addAction["<t color='#fdff01'>Haus Durchsuchen</t>",life_fnc_lootHouse,"",0,false,false,"",'isNull objectParent player && (typeOf cursorTarget in["Land_u_House_Small_01_V1_F","Land_i_Stone_HouseBig_V2_F","Land_i_Stone_HouseBig_V1_F","Land_d_House_Small_01_V1_F","Land_u_House_Big_02_V1_F","Land_i_House_Big_02_V1_F","Land_i_House_Big_02_V2_F","Land_i_House_Big_02_V3_F","Land_A_Villa_EP1","Land_e76_us_house02a","Land_e76_us_house01","Land_e76_us_house01a","Land_e76_us_house01a","Land_e76_us_house01b","Land_mbg_ger_pub_1","Land_mbg_ger_pub_2","Land_Offices_01_V1_F","Land_MilOffices_V1_F","Land_Cargo_House_V1_F","Land_Cargo_House_V2_F","Land_Cargo_House_V3_F","Land_Medevac_house_V1_F","Land_Research_house_V1_F","Land_Slum_House01_F","Land_Slum_House03_F","Land_Slum_House02_F","Land_cargo_house_slum_F","Land_i_Addon_02_V1_F","Land_i_Stone_HouseSmall_V2_F","Land_i_Stone_HouseSmall_V1_F","Land_i_Stone_HouseSmall_V3_F","Land_i_House_Small_03_V1_F","Land_i_Stone_HouseSmall_V2_F","Land_i_Stone_HouseSmall_V1_F","Land_i_Stone_HouseSmall_V3_F","Land_i_House_Small_01_V1_F","Land_i_House_Small_01_V2_F","Land_i_House_Small_01_V3_F","Land_i_Garage_V1_F","Land_i_Garage_V2_F","Land_i_House_Big_01_V1_F","Land_i_House_Big_01_V2_F","Land_i_House_Big_01_V3_F"]) && (vehicle player) distance cursorTarget < 3 && !(cursorTarget getVariable ["looted",FALSE]) && playerSide == east']);
	...
	case west:
	{
		...
		//Ausweis
		life_actions = life_actions + [player addAction["<t color='#FF00FF'>Police Badge zeigen</t>",life_fnc_copShowLicense,"",1,false,true,"",' playerSide == west && !isNull cursorTarget && player distance cursorTarget < 2.5 && cursorTarget isKindOf "Man" && alive cursortarget ']];
		...
		//gatherevidence
		life_actions pushBack (player addAction["Beweise aufsammeln",life_fnc_aufsammelnEvidence,"",0,false,false,"",' !isNull cursorTarget && player distance cursorTarget < 2.5 && !dialog && typeOf cursorTarget == "Land_Suitcase_F"']);
		...
		//Unblindfold person
		life_actions pushBack (player addAction["Augenbinde entfernen",life_fnc_untieb,"",0,false,false,"",'
		...
		(player addAction["Unverarbeitetes MDMA aufheben",life_fnc_pickupItem,false,0,false,false,"",
		...
		life_actions pushBack
		(player addAction["Verarbeitetes MDMA aufheben",life_fnc_pickupItem,true,0,false,false,"",
		...
		life_actions pushBack
		(player addAction["Unverarbeitetes Meth aufheben",life_fnc_pickupItem,false,0,false,false,"",
		...
		life_actions pushBack
		(player addAction["Verarbeitetes Meth aufheben",life_fnc_pickupItem,true,0,false,false,"",
		...
		life_actions pushBack
		(player addAction["Unverarbeitetes Kokain aufheben",life_fnc_pickupItem,false,0,false,false,"",
		...
		life_actions pushBack
		(player addAction["Verarbeitetes Kokain aufheben",life_fnc_pickupItem,true,0,false,false,"",
