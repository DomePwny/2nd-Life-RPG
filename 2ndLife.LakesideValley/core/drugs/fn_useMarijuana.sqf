...
if(life_bonging) exitwith { ["Du bist schon High genug.", false] spawn domsg; };
...
["Dieses Gras war verdammt gut, es heilt dich über Zeit...", false] spawn domsg;
...
while{_num > 0} do {
	...
	_num = _num - 1;
	if(_num == 4) then {
		["Dein Weedeffekt läuft ab in 30 Sekunden.", false] spawn domsg;
	};
	if(_num == 2) then {
		["Dein Weedeffekt läuft ab in 15 Sekunden.", false] spawn domsg;
	};
};

["Der Weedeffekt ist abgelaufen. Besorg dir neuen Stoff.", false] spawn domsg;
