...
compileFinal "
	...
	[format[""%1 überwieß dir $%2."",_from,[_val] call life_fnc_numberText], false] spawn domsg;
...
compileFinal "
	...
	if(player isEqualTo _unit && !(_vehicle in life_vehicles)) then
	{
		...
		[format[""%1 hat dir die Schlüssel für %2 gegeben."",_giver,_name], false] spawn domsg;
...
compileFinal "
	...
	ctrlShow[3015,false];
	_msg = ctrlText 3003;
	if(lbCurSel 3004 == -1) exitWith {[""Du musst eine Person auswählen!"", false] spawn domsg; ctrlShow[3015,true];};
	_to = call compile format[""%1"",(lbData[3004,(lbCurSel 3004)])];
	if(isNull _to) exitWith {ctrlShow[3015,true];};
	if(isNil ""_to"") exitWith {ctrlShow[3015,true];};
	if(_msg == """") exitWith {[""Du musst eine Nachricht eingeben!"", false] spawn domsg;ctrlShow[3015,true];};
...
compileFinal "
	...
	_msg = ctrlText 3003;
	_to = ""The Police"";
	if(_msg == """") exitWith {[""Du musst eine Nachricht eingeben!"", false] spawn domsg;ctrlShow[3016,true];};
		
	[_msg,name player,1] remoteExecCall [""clientMessage"",true];
	[] call life_fnc_cellphone;
	[format[""Du hast %1 eine Nachricht geschickt: %2"",_to,_msg], false] spawn domsg;
	ctrlShow[3016,true];
";
...
compileFinal "
	...
	_msg = ctrlText 3003;
	_to = ""The Admins"";
	if(_msg == """") exitWith {[""Du musst eine Nachricht eingeben!"", false] spawn domsg;ctrlShow[3017,true];};
...
compileFinal "
	...
	if((call life_adminlevel) < 1) exitWith {[""Du bist kein Admin!"", false] spawn domsg;};
	private[""_msg"",""_to""];
	_msg = ctrlText 3003;
	_to = call compile format[""%1"",(lbData[3004,(lbCurSel 3004)])];
	if(isNull _to) exitWith {};
	if(_msg == """") exitWith {[""Du musst eine Nachricht eingeben!"", false] spawn domsg;};
	
	[_msg,name player,3] remoteExecCall [""clientMessage"",_to];
	[] call life_fnc_cellphone;
	[format[""Admin Nachricht geschrieben an: %1 - Nachricht: %2"",name _to,_msg], false] spawn domsg;
";
...
compileFinal "
	...
	[] call life_fnc_cellphone;
	[format[""Admin Nachricht an alle: %1"",_msg], false] spawn domsg;
";
...
compileFinal "
	...
	switch (_type) do
	{
		case 0 :
		{
			...
			hint parseText format [""<t color='#FFCC00'><t size='2'><t align='center'>Neue SMS<br/><br/><t color='#33CC33'><t align='left'><t size='1'>an: <t color='#ffffff'>dich<br/><t color='#33CC33'>From: <t color='#ffffff'>%1<br/><br/><t color='#33CC33'>SMS:<br/><t color='#ffffff'>%2"",_from,_msg];
