...
*/
private["_mode","_spawnPoints","_className","_basePrice","_colorIndex","_spawnPoint","_vehicle","_newPrice"];
_mode = _this select 0;
if((lbCurSel 2302) == -1) exitWith {hint localize "STR_Shop_Veh_DidntPick"};
_className = lbData[2302,(lbCurSel 2302)];
_vIndex = lbValue[2302,(lbCurSel 2302)];
_vehicleList = [life_veh_shop select 0] call life_fnc_vehicleListCfg; _basePrice = (_vehicleList select _vIndex) select 1;
 if(!_mode) then {_basePrice = round(_basePrice / 3)};
...
if(playerSide != civilian) then {
	...
	man_ey_b4nK = man_ey_b4nK - _newPrice;
} else {
...
//	_vehicle disableTIEquipment true; //No Thermals.. They're cheap but addictive.
	...
	if(!(_vehicle isKindOf "Ship")) then {
		_vehicle setPosATL (_vehicle modelToWorld [0,0,0.65]);
	};
	[[_vehicle,_colorIndex],"life_fnc_colorVehicle",true,false] spawn life_fnc_MP;
	[_vehicle] call life_fnc_clearVehicleAmmo;
	[[_vehicle,"trunk_in_use",false,true],"TON_fnc_setObjVar",false,false] spawn life_fnc_MP;
