...
while {true} do
{
	if(animationState player != "Acts_TreatingWounded01") then {
		[[player,"Acts_TreatingWounded01"],"life_fnc_animSync",true,false] spawn life_fnc_MP;
...
if(_target getVariable ["Reviving",ObjNull] != player) exitWith {life_action_inUse = false; hint localize "STR_Medic_AlreadyReviving"};
...
man_ey_b4nK = man_ey_b4nK + (call life_revive_fee);
