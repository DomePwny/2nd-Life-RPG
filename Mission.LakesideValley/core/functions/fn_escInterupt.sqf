...
_canUseControls = {
	...
	if((player getVariable["restrained",FALSE]) OR (player getVariable["Escorting",FALSE]) OR (player getVariable["transporting",FALSE]) OR (life_istazed) OR (lrl_knockedOut)) then {false} else {true};
};
...
while {true} do
{
	...
	_abortButton = (findDisplay 49) displayCtrl 104;
	_abortButton buttonSetAction "[[player],""TON_fnc_cleanupRequest"",false,false] spawn life_fnc_MP; call SOCK_fnc_updateRequest;";
...
/*	if(_usebleCtrl) then {
		_respawnButton ctrlEnable true; //Enable the button. - Disable this roleplay-destroying motherfcking 'respawn' shit!
	};*/
	waitUntil{isNull (findDisplay 49)};
};
